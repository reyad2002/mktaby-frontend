...
export async function getClientResources(
  id: number | string,
  queryParams: ClientResourcesQuery
): Promise<GetClientResourcesResponse> {
  const response = await apiClient.get<GetClientResourcesResponse>(
    CLIENT_RESOURCES_PATH(id),
    {
      params: queryParams,
    }
  );
